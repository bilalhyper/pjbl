<?php
    // Mendapatkan data cart dari localStorage (jika ada)
...
<body>
    ...
    <section class="cart-section bg-light">
        <div class="container">
            <h2>Order   | <a href="cart.php"> <span class="badge ">Cart</span></a></h2> 
        ...
        <div class="text-center">
            <button id="complete-order-btn" class="btn btn-success">Cancel Order</button>
...
<script>
    // Mengambil informasi produk yang telah dibeli dari localStorage
    const selectedProduct = JSON.parse(localStorage.getItem('selectedProduct'));

    if (selectedProduct) {
        const orderDetails = document.getElementById('order-details');
        const productTotal = parseFloat(selectedProduct.price) * selectedProduct.quantity;
        
        // Menampilkan detail produk yang sudah dibeli
        orderDetails.innerHTML = `
            <tr>
                <td><img src="${selectedProduct.image}" alt="Product Image" width="100"></td>
                <td>${selectedProduct.name}</td>
                <td>${selectedProduct.size}</td>
                <td>${selectedProduct.quantity}</td>
                <td>Rp ${parseFloat(selectedProduct.price).toLocaleString('id-ID', { minimumFractionDigits: 2 })}</td>
                <td>Rp ${productTotal.toLocaleString('id-ID', { minimumFractionDigits: 2 })}</td>
            </tr>
        `;
    } else {
        const orderDetails = document.getElementById('order-details');
        orderDetails.innerHTML = '<tr><td colspan="6">No order details available.</td></tr>';
    }

    // Fungsi untuk menghapus produk setelah order selesai
    document.getElementById('complete-order-btn').addEventListener('click', function() {
        localStorage.removeItem('selectedProduct'); // Hapus produk setelah order selesai
        alert("Order completed successfully!");
        window.location.href = "index.php"; // Redirect ke halaman home atau halaman lain
    });
</script>
</body>
</html>